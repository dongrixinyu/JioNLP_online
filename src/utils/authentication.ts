...
function authentication_hash_code(text: string) {
  ...
  const random_int = 30000 + Math.round(random_num * 20000);
  // console.log("random_int");
  // console.log(random_int);  // 范围在 30000 ~ 50000

  if (text == "") {
    whole_text =
      random_int.toString() + "-" + "-".repeat(text_length) + "-" + api_id;
  } else {
    whole_text =
      random_int.toString() + "-" + text.slice(0, text_length) + "-" + api_id;
  }

  const hash_code = Md5.hashStr(whole_text);
